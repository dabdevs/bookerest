...
export default function EditButton({ className = '', disabled, ...props }) {
    ...
    return (
            <button
                ...
                type={'button'} 
                className={
                    `inline-flex px-2 items-center ${size} bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-500 active:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150 ${disabled && 'opacity-25'
                    } ` + className
                }
                disabled={disabled}
            >
